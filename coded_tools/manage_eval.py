...
class ManageEvalMulti(CodedTool):
    """
        ...
        if args is not None:
            for key in self.eval_data:
                if key in args:
                    ...
                    updated_evaluation[key] = ManageEvalMulti.compute_average(args[key])
    ...
    def compute_average(list_of_scores: List[Union[str, int, float, None]]) -> float:
        ...
        numeric_values = [float(v) for v in list_of_scores if ManageEvalMulti.is_valid_number(v)]
